...
const buildWhereClause = (filter, tableAlias = '') => {
  const conditions = [];
  const params = [];
  let paramIndex = 1;
  ...
  if (filter.tenantId) {
    conditions.push(`${prefix}tenant_id = $${paramIndex}`);
...
// CORRIGIDO: alias 'l' em todas as referências para evitar "column reference tenant_id is ambiguous"
const getInactiveLeads = async (filter, days = 7) => {
    // Passa alias 'l' para o buildWhereClause prefixar corretamente
    const { clause, params } = buildWhereClause(filter, 'l');
    const result = await pool.query(
        `SELECT 
            l.*,
